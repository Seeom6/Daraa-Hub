...
export class CourierEventsListener {
  ...
  async handleDeliveryStatusUpdate(payload: {
    ...
    this.logger.log(
      `Handling delivery status update: ${payload.orderId} -> ${payload.status}`,
    );
    ...
    if (payload.status === 'delivered') {
      try {
        ...
        if (payment && payment.paymentMethod === PaymentMethodType.CASH) {
          this.logger.log(
            `Confirming cash payment for order ${payload.orderId}`,
          );
          ...
          this.logger.log(
            `Cash payment confirmed for order ${payload.orderId}`,
          );
        }
      } catch (error) {
        this.logger.error(
          `Failed to confirm cash payment for order ${payload.orderId}:`,
          error,
        );
      }
    }
  ...
  async handleOrderAssigned(payload: {
    ...
    this.logger.log(
      `Order ${payload.orderId} assigned to courier ${payload.courierId}`,
    );
  ...
  async handleOrderAccepted(payload: {
    ...
    this.logger.log(
      `Courier ${payload.courierId} accepted order ${payload.orderId}`,
    );
  ...
  async handleOrderRejected(payload: {
    ...
    this.logger.log(
      `Courier ${payload.courierId} rejected order ${payload.orderId}: ${payload.reason}`,
    );
  ...
  async handleCourierSuspended(payload: {
    ...
    this.logger.log(
      `Courier ${payload.courierId} suspended by ${payload.suspendedBy}: ${payload.reason}`,
    );
    // TODO: Send notification to courier
  }
  ...
  async handleCourierUnsuspended(payload: { courierId: string }) {
